...
export function DataTable<TData, TValue>({
  ...
  const dataIds: UniqueIdentifier[] = table.getRowModel().rows.map((row) => (isNaN(Number(row.id)) ? row.id : Number(row.id)) as UniqueIdentifier);
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    if (active && over && active.id !== over.id && onReorder) {
      ...
      const newData = arrayMove(table.options.data as TData[], oldIndex, newIndex);
      onReorder(newData);
    }
  }
